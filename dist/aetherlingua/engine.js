...
import { SumerianTokenizer } from './tokenizers/sumerian';
export class AetherLingua {
     ...
     */
    renderKemeticInvocation(lines, outputDir = '.') {
        const buffers = [];
        ...
        // Write WAV files
        const filenames = WavWriter.writeMultiple(`${outputDir}/aetherlingua_kemetic.wav`, buffers, this.config.sampleRate);
        return filenames;
    }
    /**
     * Render Sumerian cuneiform invocation with percussive transients
     */
    renderSumerianInvocation(glyphText, outputFile) {
        ...
        for (const glyph of glyphs) {
            const strikeTimings = SumerianTokenizer.generateRhythmicPattern(glyph.wedgeCount, glyphDuration);
            for (const strikeTime of strikeTimings) {
                ...
                for (let i = 0; i < n && startSample + i < n; i++) {
                    const t = i * step;
                    const strike = SumerianTokenizer.generateWedgeStrike(t, glyph.percussiveIntensity, glyph.bronzeBellFreq);
     ...
     */
    renderHybridInvocation(kemeticLines, sumerianGlyphs, outputFile) {
        const n = Math.floor(this.config.duration * this.config.sampleRate);
        const left = new Float32Array(n);
        const right = new Float32Array(n);
        ...
        for (const glyph of glyphs) {
            ...
            for (const strikeTime of strikeTimings) {
                ...
                for (let i = 0; i < n && startSample + i < n; i++) {
                    const t = i * step;
                    const strike = SumerianTokenizer.generateWedgeStrike(t, glyph.percussiveIntensity * 0.4, glyph.bronzeBellFreq);
                    const pan = 0.5 + Math.sin(absoluteTime * 0.5) * 0.3;
                    left[startSample + i] += strike * (1.0 - pan);
                    right[startSample + i] += strike * pan;
                }
