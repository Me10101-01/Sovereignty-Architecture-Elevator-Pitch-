...
export class Oscillators {
     ...
     */
    static sine(frequency, time) {
        return Math.sin(2.0 * Math.PI * frequency * time);
    }
    /**
     * Sawtooth wave oscillator
     */
    static saw(frequency, time) {
     ...
     */
    static triangle(frequency, time) {
        const phase = frequency * time - Math.floor(frequency * time);
     ...
     */
    static charityGliss(carrier, time) {
        if (time < 1.2) {
     ...
     */
    static node137Burst(carrier, time) {
        if (time < 0.8) {
