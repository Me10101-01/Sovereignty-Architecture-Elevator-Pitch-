...
This Autonomous Operation License operates within the framework established by:

1. **NON_AGGRESSION_CLAUSE.md** (IMMUTABLE) - All autonomous actions must comply
2. **TRUST_DECLARATION.md** - Foundational trust principles apply
3. **access_matrix.yaml** - Role-based permissions remain in effect
4. **auto_approve_config.yaml** - Technical automation patterns are compatible

### Hierarchy of Authority
```
IMMUTABLE CONSTRAINTS:
├── NON_AGGRESSION_CLAUSE.md (cannot be overridden)
│
AUTONOMOUS OPERATION:
├── AUTONOMOUS_OPERATION_LICENSE.md (this document)
│   ├── Pre-authorized actions within ethical boundaries
│   ├── Circuit breakers enforce compliance
│   └── Audit trail ensures accountability
│
TECHNICAL IMPLEMENTATION:
├── aol_config.yaml (machine-readable rules)
├── aol_circuit_breakers.yaml (thresholds and triggers)
└── aol_audit_schema.yaml (logging requirements)
```
...
- Board consensus on any edge cases
...
### Annual Renewal
- Complete audit of all operations during license period
- Zero critical incident verification
- Operator re-authorization for next period
...
**Related Documents:** NON_AGGRESSION_CLAUSE.md, TRUST_DECLARATION.md, access_matrix.yaml
