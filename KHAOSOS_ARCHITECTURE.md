...
See `configs/nixos/khaosos-configuration.nix` for the complete declarative configuration.

**Key Features:**
- Hardened kernel with security sysctls
- Default-deny firewall configuration
- GPG and age encryption tools
- WireGuard and Tailscale mesh networking
- Docker and Kubernetes tooling
- Ollama for local LLM inference
- Complete sovereign tool stack
...
KhaosSearch provides a self-hosted meta-search engine that aggregates results from multiple search providers without tracking or vendor lock-in.

See `docker-compose.khaossearch.yml` for deployment configuration.
...
**Air-Gapped Inference Capability:**
- Local model execution with Ollama
- No internet connectivity required for inference
- Encrypted input/output via USB for air-gapped scenarios
- Full sovereignty over AI operations

**Models:**
- `qwen2.5:72b` - Sovereign node (primary)
- `llama3.2:70b` - General purpose
- `codellama:34b` - Code generation
- `mistral:7b` - Fast inference

**Infrastructure:**
- Qdrant for vector storage (empire_memory, antibodies, knowledge_graph)
- Redis for session cache and rate limiting
- GPU acceleration (NVIDIA L4 or RTX 4090)
...
The Queen CLI provides a secure, GPG-signed command interface for managing the entire Strategickhaos empire.

See `QUEEN_CLI.md` for complete documentation.
...
See `KHAOSOS_ROADMAP.md` for detailed implementation phases.
...
- `docker-compose.khaossearch.yml` - KhaosSearch deployment
- `QUEEN_CLI.md` - Queen CLI documentation
- `KHAOSOS_ROADMAP.md` - Implementation roadmap
- `configs/khaos-tools/` - Individual tool specifications
...
**Timestamp:** 2025-12-07T22:11:00Z
